...
/**
 * Resolves the ESLint config governing a run and imports the elements that it exports, reporting rather than throwing
 * when it cannot. ESLint reads the same file through jiti, which transpiles; strict-lint reads it through Node's type
 * stripping, which does not, so a config that Node rejects skips the check rather than failing a run that ESLint
 * completed.
 */
...
/**
 * Checks whether a value is an array of config objects. It verifies only what the comparison depends on -- that each
 * element is an object whose identity and `rules` can be read -- and leaves the rest to ESLint, which has already
 * loaded the same file and would have rejected a malformed element.
 */
...
/** Reads the default export, resolving the promise form that ESLint accepts and flattening as ESLint does. */
async function readDefaultExport(filePath: string): Promise<unknown> {
  ...
  // Normalize as ESLint does, which takes a lone config object as a one-element array and flattens a nested array.
