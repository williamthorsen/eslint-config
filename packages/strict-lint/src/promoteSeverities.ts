...
/** Counts the messages at a severity. */
...
/** Counts the fixable messages at a severity. */
...
/**
 * Returns a message's severity after promotion: `error` for a warning that no ceiling caps, and its original severity
 * otherwise. A message naming no rule keeps its severity, because `maxSeverity` has rule names as keys and promoting
 * such a message would raise an error that no ceiling could exempt. ESLint reports both the ignored-file
