...
- 🚨 **Breaking:** Publish a toolIgnores list and correct commonIgnores (#217)

  - Adds `toolIgnores`, a list of patterns for content generated by developer tools: Claude, ReadyUp, and Rovo.
  - Corrects `commonIgnores` to cover build output and files no rule can usefully report on, dropping `**/lib/**`, `**/output/**`, and `*.d.ts`, and adding `**/*.sh`.

  Migration: Add `**/lib/**`, `**/output/**`, or `*.d.ts` to the consuming config's own ignore list wherever the path is genuinely a build target, since ESLint now reports on every such file that `commonIgnores` no longer covers.
