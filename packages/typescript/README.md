...
The `readyup >=0.33.0` peer names the version the kit is developed and tested against. The kit runs below it too: readyup does not report the repo root among a monorepo's workspaces until 0.33.0, so the kit supplies the root itself and sweeps every member package alongside it.
