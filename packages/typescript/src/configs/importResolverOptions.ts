// The bundled resolver defaults `extensions` to `['.mjs', '.cjs', '.js', '.json', '.node']` and sets no
...
// Each alias list ends with the JavaScript extension itself, which keeps a genuine JavaScript sibling
// resolvable; the alias replaces the search extension rather than adding to it. Both lists are ordered by
// resolution priority rather than alphabetically: When a `.js` and a `.ts` sibling both exist, an
