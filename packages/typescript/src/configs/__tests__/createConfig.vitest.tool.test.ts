...
// No parser override, so the fixture is parsed by espree and has no parser services.
...
// option or changes a default. Each fixture also contains one expected report to distinguish the absence of the
// silenced rules from a config that stopped matching the fixture, which would report nothing either way.
describe('createConfig.vitest rule corrections', () => {
  it('reports only the untouched rule on the patterns that the corrections permit', async () => {
    const composed = [fixtureWiring, ...(await createConfig.vitest())];

    const results = await lintFixture(composed, 'vitest-tuned.test.ts');

    // Map to rule ids so that a regression names the rule that fired instead of reporting a length mismatch.
    expect(results[0]?.messages.map((message) => message.ruleId)).toStrictEqual(['vitest/prefer-to-be']);
  });

  // The type-aware rules abort the run rather than degrading, so a carve-out regression appears here as a thrown
  // "requires type information" error out of `lintFixture`. The expected report separates that from a config that
  // stopped matching the fixture, which would also produce no throw and no messages.
...
// test edit. The default export contains a test-scoped rule block, which makes it the nearest config to a report on a
// test file.
describe('vitest rule ownership', () => {
  it('sets vitest rule severities in no published config but its own', async () => {
