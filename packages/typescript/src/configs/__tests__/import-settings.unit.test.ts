...
describe('the extensions that the import config settings name', () => {
  it('match the extensions that its file patterns cover', () => {
...
/** Expands a glob brace group such as `{js,cjs}` into the dotted extensions that it covers. */
...
/** Collects the extensions that the config's `import-x/extensions` setting names, empty when no block sets it. */
