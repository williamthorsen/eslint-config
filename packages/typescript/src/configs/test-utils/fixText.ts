...
export interface FixResult {
  ...
  /** The fixed text, or the input when no fix applied. */
  output: string;
}

// Assigns a file that exists only as text to the fixture tsconfig so that type-aware rules run against it and
// its imports of fixture modules resolve. `projectService: true` alone admits only files that the project lists.
