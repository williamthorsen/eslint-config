...
- **A readyup kit spans two trees.** Its predicates live in the workspace's `src/readiness/`, where tests and coverage reach them; `.readyup/kits/default.ts` holds only the kit declaration and the filesystem wrappers it composes them into, and `rdy compile` bundles the two. Putting a `__tests__/` under `.readyup/` instead would collect nothing, because tinyglobby does not traverse dot-directories: the files report as zero tests rather than as an error. A kit's `export default` is required by readyup and is the only one either workspace carries. `.config/readyup.config.ts` names seven packages, including this repo's own `@williamthorsen/tsconfig` and `@williamthorsen/eslint-config-typescript`, so the repo runs each of its kits against itself. The latter's extends check matches the package specifier as text or a path into a workspace providing the package, which is what lets `eslint.config.ts` satisfy it by importing `packages/typescript/src/` directly.
